...
struct DesktopLyricsView: View {
    ...
    var body: some View {
        Group {
            if store.settings.desktopLyricsVerticalLayout {
                ...
                horizontalLyricsContent
            }
        }
        .overlay {
            if store.isDesktopLyricsDragging {
                RoundedRectangle(
                    cornerRadius: max(8, store.settings.desktopLyricsFontSize * 0.35),
                    style: .continuous
                )
    ...
    private var verticalLyricsContent: some View {
        let lines = store.desktopLyricsDisplayLines()
        let fontSize = store.settings.desktopLyricsFontSize
        let direction = store.settings.desktopLyricsVerticalDirection
        let alignment = store.settings.desktopLyricsAlignment
        let verticalPadding = DesktopLyricsLayout.verticalPadding(for: fontSize)
        let columnWidth = DesktopLyricsLayout.verticalColumnWidth(for: fontSize)
        let columnSlotWidth = DesktopLyricsLayout.verticalColumnSlotWidth(for: fontSize)
        let contentWidth = DesktopLyricsLayout.verticalTotalWidth(for: fontSize, lineCount: lines.count)
            - DesktopLyricsLayout.horizontalPadding(for: fontSize) * 2

        return GeometryReader { proxy in
            let viewportHeight = max(1, proxy.size.height - verticalPadding * 2)
            ZStack(alignment: .topLeading) {
                ForEach(Array(lines.enumerated()), id: \.element.id) { offset, line in
                    let columnOffset = direction == .leftToRight
                        ? offset
                        : lines.count - 1 - offset
                    VerticalDesktopLyricColumn(
                        line: line,
                        palette: store.desktopLyricsPalette(),
                        fontSize: fontSize,
                        viewportHeight: viewportHeight,
                        alignment: alignment
                    )
                    .frame(width: columnWidth, height: viewportHeight, alignment: .top)
                    .offset(x: Double(columnOffset) * columnSlotWidth)
                }
            }
            .frame(width: contentWidth, height: viewportHeight, alignment: .topLeading)
            .padding(.horizontal, DesktopLyricsLayout.horizontalPadding(for: fontSize))
            .padding(.vertical, verticalPadding)
...
private struct VerticalDesktopLyricColumn: View {
    ...
    var body: some View {
        ...
        .frame(width: DesktopLyricsLayout.verticalColumnWidth(for: fontSize), height: viewportHeight)
            ...
            // Keep the edge selected by the vertical alignment fixed while an
            // upcoming column is de-emphasised. With a top anchor, an inactive
            // column's bottom edge floated upward in Bottom mode.
            .scaleEffect(line.isActive ? 1 : 0.88, anchor: alignment.verticalScaleAnchor)
            .blur(radius: line.isActive ? 0 : 0.4)
            .transition(.opacity.combined(with: .scale(scale: 0.92, anchor: .top)))
            .animation(promotionAnimation, value: line.isActive)
            .onAppear { syncPlaybackAnchor(to: line.playbackTime) }
            .onChange(of: line.id) { _, _ in
                syncPlaybackAnchor(to: line.playbackTime)
            }
            .onChange(of: line.playbackTime) { _, playbackTime in
                syncPlaybackAnchor(to: playbackTime)
            }
            .onChange(of: line.isPlaying) { _, _ in
                syncPlaybackAnchor(to: line.playbackTime)
            }
    }
